...
teams/{teamId}/dailyInstances/{date}/submissions/{submissionId}   ← doc ID = {uid}_{activityId}
...
### `teams/{teamId}/dailyInstances/{date}/submissions/{submissionId}`

One per member per activity per day. Document ID = `{uid}_{activityId}` (e.g. `abc123_goalXYZ`).

```typescript
submissionId: string          // = document ID; client merges from Firestore doc ID
teamId: string                // denormalized
date: string                  // denormalized
submitterUid: string
activityId: string            // goal document ID for stable identity
displayName: string           // denormalized
avatarAssetName: string       // denormalized
activityName: string          // display name of the activity
photoURL: string              // short-lived signed URL (7-day); generated by CF
photoPath: string             // "proof-photos/{teamId}/{date}/{uid}_{activityId}.jpg"
...
* **No self-approval:** security rule enforces voter is not the submission's `submitterUid`.
...
```text
proof-photos/{teamId}/{date}/{uid}_{activityId}.jpg
```
