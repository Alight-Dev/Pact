...
| `Pact/Upload/UploadView.swift` | Upload coordinator (`UploadProofView`). Checks camera permission on appear; shows `CameraPermissionExplainerView` if needed, then `CameraScreen` for capture, then `ConfirmPhotoView` for review/submit. No pre-photo static page. |
| `Pact/Upload/CameraScreen.swift` | Full-screen custom camera (AVCaptureSession). Includes live preview, a Snapchat-style activity carousel (snap to center, selected item highlighted), flash toggle (torch in preview + flashMode on capture), front/back flip, and a shutter button. Flash auto-disables on front camera. |
| `Pact/Upload/ConfirmPhotoView.swift` | Dark immersive proof-confirm screen. Photo fills the display with a gradient overlay; selected activity icon + name displayed above Retake / Submit buttons. Submit calls `firestoreService.submitProof` and dismisses the sheet on success. |
